classdef ANFIS
    ...
    %
    methods (Static)
        function Model=train(FMWR11c,two_out,split_range,numMFs,inmftype,outmftype,dispOpt,epoch_n)
            ...
            while(1)
                if iteration==1
                    [Model(iteration).AnfisModel,Model(iteration).Reference,Model(iteration).splitrange]=ANFIS.subtrain(FMWR11c,two_out,...
                    ...
                    [Model(iteration).AnfisModel,Model(iteration).Reference,Model(iteration).splitrange]=ANFIS.subtrain(Model(iteration-1).Reference,...
                        two_out,split_range,inmftype,outmftype,numMFs,dispOpt,epoch_n );
                end
        ...
        function [AnfisModel,Reference,splitrange]=subtrain(FMWR11c,two_out,split_range,mfType1,mfType2,numMFs,dispOpt,epoch_n )
            %% Split data for Better Classification
            lengthOfdata=size(FMWR11c,2);
            ...
            Reference=zeros(size(FMWR11c,1),cycle);
            for i=1:cycle
                dataRange=splitrange(i)+1:splitrange(i+1);
                fisdata=[FMWR11c(:,dataRange) two_out];
                fis1=genfis1(fisdata,numMFs,mfType1,mfType2);
                AnfisModel{i}=anfis(fisdata,fis1,epoch_n,dispOpt);
                Reference(:,i)=evalfis(FMWR11c(:,dataRange),AnfisModel{i});
            end
        end
        
        function Result=classify(Model,two_out)
            % Inputs
            ...
            for iteration=1:length(Model)
                ...
                if iteration==1
                    Result=zeros(size(two_out,1),length(splitrange)-1);
                    for i=1:length(splitrange)-1
                        range=splitrange(i)+1:splitrange(i+1);
                        Result(:,i)=evalfis(two_out(:,range),Model(iteration).AnfisModel{i});
                    end
                else
                    ResultOld=Result;
                    Result=zeros(size(two_out,1),length(splitrange)-1);
